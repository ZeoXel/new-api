...
输入给模型，生成视频的信息，支持文本信息、图片信息和样片信息。支持的组合形式如下：

* **填写文本信息**：基于文本提示词生成正常视频。
* **填写文本信息和图片信息**：基于文本提示词和图片生成正常视频。
* **填写样片信息==^new^==** ** ** ：基于已成功生成的 Draft 视频，生成正常的高质量视频。
...
:::warning **adaptive 使用说明**
当使用 **adaptive** 时，模型会根据不同的视频生成场景自适应调整视频的宽高比；实际生成的视频宽高比可通过 [查询视频生成任务 API](https://www.volcengine.com/docs/82379/1521309?lang=zh) 返回的 **ratio** 字段获取。

* 文生视频场景：根据输入的提示词，自动选择最合适的宽高比。
* 图生视频场景：根据所上传首帧图片的比例，自动选择最合适的宽高比。
* **参考图生视频场景、Seedance 1.0 系列的文生视频场景均不支持使用 adaptive。** 
